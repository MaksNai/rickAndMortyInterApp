...
export function MainLocations() {
  ...
  // Select fields
  const typeOptions = useMemo(() => getUniqueValues(allLocations, 'type'), [allLocations])
  const dimensionOptions = useMemo(() => getUniqueValues(allLocations, 'dimension'), [allLocations])
  ...
  const content = useMemo(() => {
    ...
    if (locations.length < itemsPerPage && currentPage !== maxPage) setIsNeedMore(true)
  ...
  return (
    <main className={styles.main}>
      ...
      >
        {currentPage <= maxPage && hasMore && locations.length > 0 && <LoadMoreButton />}
