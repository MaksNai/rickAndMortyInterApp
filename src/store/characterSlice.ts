...
function isCharacter(payload: FetchCharactersPayload | Character): payload is Character {
...
export const { setCharacterFilter, resetCharacterFilters } = charactersSlice.actions
...
>(
  ...
  (characters: Character[], characterIds: string[]) =>
    characters.filter((character) => characterIds.includes(character.id.toString())),
)
