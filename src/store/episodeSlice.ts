...
export const fetchEpisodesByIds = createAsyncThunk<FetchEpisodePayload, EpisodeUrls>(
    ...
    builder
      ...
      .addCase(fetchEpisodes.fulfilled, (state, action) => {
        ...
        if (action.payload.results && Array.isArray(action.payload.results)) {
          ...
          console.error('Unexpected payload structure in fetchEpisodes:', action.payload)
        }
...
>([selectAllEpisodes, selectEpisodeFilters], (episodes, filters) =>
  episodes.filter((episode) => episode.name.toLowerCase().includes(filters.name.toLowerCase())),
)

export const selectEpisodesByIds = createSelector<
  [typeof selectAllEpisodes, (state: AppState, episodeIds: string[]) => string[]],
  Episode[]
>(
