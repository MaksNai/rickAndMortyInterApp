...
export const fetchLocationsByIds = createAsyncThunk<FetchLocationPayload, LocationUrls>(
  ...
  async (locationIds) => {
    ...
    const response = await axios.get(`https://rickandmortyapi.com/api/location/${ids}`)
    return response.data as FetchLocationPayload
  },
)
...
export const selectAllLocations = (state: AppState): Location[] => state.locations.entities

export const selectLocationsFilters = (state: AppState): LocationState['filters'] =>
  state.locations.filters

export const selectFilteredLocations = createSelector<
  [(state: AppState) => Location[], (state: AppState) => LocationState['filters']],
  Location[]
>([selectAllLocations, selectLocationsFilters], (locations, filters) =>
  locations.filter(
    (location) =>
      (!filters.name || location.name.toLowerCase().includes(filters.name.toLowerCase())) &&
